...
class Route {
  ...
  async getEstimateTimeByStopId(routeId, direction, stopId) {
    try {
			return await axios({
				url: `${baseUrl}/Bus/EstimatedTimeOfArrival/City/Taichung?$filter=RouteUID%20eq%20%27${routeId}%27%20and%20Direction%20eq%20%27${direction}%27%20and%20StopID%20eq%20%27${stopId}%27&$format=json`,
				method: 'get',
				headers: await getAuthorizationHeader(),
			});
		} catch (error) {
  ...
  async getAllEstimateTimeByRouteId(routeId, direction) {
		try {
			console.log(routeId)
			return await axios({
				url: `${baseUrl}/Bus/EstimatedTimeOfArrival/City/Taichung?$filter=RouteUID%20eq%20%27${routeId}%27%20and%20Direction%20eq%20%27${direction}%27&$orderby=StopSequence&$format=JSON`,
				method: 'get',
				headers: await getAuthorizationHeader(),
			});
		} catch (error) {
